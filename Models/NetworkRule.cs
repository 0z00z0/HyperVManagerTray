...
/// <summary>
/// A switch and the VMs to connect to it — what a rule and the fallback both carry. Identified by
/// <see cref="SwitchId"/> and <see cref="TargetVmIds"/>; the rest is shown or migrated, never matched on.
/// </summary>
public interface ISwitchTarget
{
    /// <summary>The virtual switch ID Hyper-V assigns (a GUID). Empty while not identified yet.</summary>
    string SwitchId { get; set; }

    /// <summary>The switch's name as last seen. Shown only.</summary>
    string SwitchName { get; set; }

    /// <summary>A switch NAME from a settings document written before identifiers existed. Present only
    /// until it is identified; while it is, the entry needs attention.</summary>
    string? LegacyVirtualSwitch { get; set; }

    /// <summary>The VM IDs of the managed VMs to reconnect to the switch.</summary>
    List<string> TargetVmIds { get; set; }

    /// <summary>VM NAMES from an older document that are not identified yet. Each one needs attention.</summary>
    List<string>? LegacyTargetVms { get; set; }
}
...
/// </summary>
public sealed class NetworkRule : ISwitchTarget
{
    /// <summary>What identifies the rule: a GUID the app generates. The name is only shown, so two rules
    /// may share one.</summary>
    public string Id { get; set; } = string.Empty;

    /// <summary>Set when <see cref="Id"/> was generated by the load rather than read from the file, so the
    /// app knows to write it down. Never serialised.</summary>
    [JsonIgnore]
    public bool IdGeneratedOnLoad { get; set; }
    ...
    /// <summary>
    /// When true, the <see cref="TargetVmIds"/> are started (or resumed if paused) the moment this
