...
|---|---|---|---|---|
| [Microsoft.WindowsAppSDK](https://www.nuget.org/packages/Microsoft.WindowsAppSDK) | 2.5.1 | Microsoft | WinUI 3 framework (windowing, XAML, Mica) | MS-EULA¹ |
...
| [ZeroZero.Update.WinUI](https://github.com/0z00z0/0z0-shared) | 0.11.0 | The self-update flow behind `Services/AppUpdate.cs` — the release check, the download verified against its published SHA-256 and against the publisher certificate before it is allowed to run, and the one window that carries the question, the download's progress and the answer. The component can be told to accept a self-signed release on the publisher name alone; this app does not ask for that, so the pinned certificate is still required. Delivers `ZeroZero.Update.Win32` and `ZeroZero.Update` with it |
| [ZeroZero.Win32](https://github.com/0z00z0/0z0-shared) | 0.9.1 | The count of this app's own short-lived windows, asked by the dashboard before it hides itself on lost focus so the update window does not close it |
