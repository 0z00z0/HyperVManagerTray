...
| [ZeroZero.Update.Win32](https://github.com/0z00z0/0z0-shared) | 0.9.0 | The self-update flow behind `Services/AppUpdate.cs` — the release check, and the download verified against its published SHA-256 and against the publisher certificate before it is allowed to run. The component can be told to accept a self-signed release on the publisher name alone; this app does not ask for that, so the pinned certificate is still required. Delivers `ZeroZero.Update` with it |
